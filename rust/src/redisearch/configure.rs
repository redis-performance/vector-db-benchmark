...
impl RustRedisConfigurator {
        ...
        match drop_result {
            ...
            Err(e) => {
                    ...
                    || err_str.contains("no such index")
                {
                    ...
                    println!("Given the FT.DROPINDEX command failed, we're flushing the entire DB...");
                    redis::cmd("FLUSHALL")
                        .query::<()>(&mut conn)
                        .map_err(|e| {
    ...
    fn recreate(&self, dataset: &Bound<'_, PyAny>, collection_params: &Bound<'_, PyAny>) -> PyResult<()> {
        self.clean()?;
        ...
        let algo_config = collection_params
            .call_method1("get", (config_key.as_str(), PyDict::new_bound(py)))?;
