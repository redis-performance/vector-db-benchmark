...
use crate::redisearch::parser::{
    extract_meta_conditions, MetaConditions, RedisConditionParser,
};
...
impl RustRedisSearcher {
    ...
    fn init_client(
        ...
        let inner_params = search_params.call_method1(
            "get",
            ("search_params", Python::with_gil(|py| py.None())),
        )?;

        let ef: Option<i64> = if !inner_params.is_none() {
            let ef_val = inner_params.call_method1("get", ("ef", Python::with_gil(|py| py.None())))?;
            if ef_val.is_none() {
                None
            } else {
                Some(ef_val.extract()?)
    ...
    fn search_all(
        ...
        let search_params = Python::with_gil(|py| -> PyResult<SearchConfig> {
            ...
            let ef: Option<i64> = if !inner_params.is_none() {
                ...
                if ef_val.is_none() { None } else { Some(ef_val.extract()?) }
            } else {
                None
            };
        ...
        let rust_queries: Vec<RustQuery> =
            Python::with_gil(|_py| -> PyResult<Vec<RustQuery>> {
                ...
                loop {
                    match queries_iter.call_method0("__next__") {
                        Ok(query) => {
                            ...
                            let vector: Vec<u8> =
                                if let Ok(bytes) = vector_obj.extract::<Vec<u8>>() {
                                    bytes
                                } else {
        ...
        let results: Vec<SearchResult> =
            Python::with_gil(|py| -> PyResult<Vec<SearchResult>> {
                py.allow_threads(|| {
                    if parallel <= 1 {
...
fn run_sequential(
    ...
    let config = RedisConfig::from_env();
    let mut conn =
        create_connection(host, &config).map_err(|e| format!("Redis error: {}", e))?;
