...
/// Metadata extracted from Python dict, ready for use without GIL
#[derive(Clone)]
struct MetadataItem {
...
fn extract_metadata_item(meta_obj: &Bound<'_, PyAny>) -> PyResult<Option<MetadataItem>> {
    if meta_obj.is_none() {
        return Ok(None);
    }
        ...
        loop {
            match iter.call_method0("__next__") {
                Ok(kv) => {
                    ...
                    if k == "labels" {
                        if let Ok(label_list) = v.extract::<Vec<String>>() {
                            fields.push((k, MetadataValue::Labels(label_list)));
                        }
                        continue;
                    }
                    ...
                    if v.is_instance_of::<pyo3::types::PyDict>() {
                        ...
                        let lat = v
                            ...
                            .and_then(|l| l.extract::<f64>())
                            .unwrap_or(0.0);
                        fields.push((k, MetadataValue::Geo { lon, lat }));
                        continue;
                    }
                    ...
                    // Scalar values (int, float, string)
                    let val_str: String = v.str()?.to_string();
                    fields.push((k, MetadataValue::String(val_str)));
                }
                Err(_) => break,
            }
        }
