...
use crate::readers::{
    read_compound_vectors_only, read_hdf5_vectors, read_jsonl_vectors, read_npy_vectors,
};
...
impl RustVsetUploader {
    ...
    fn init_client(
        ...
        let hnsw_config = upload_params
            .getattr("__getitem__")?
            .call1(("hnsw_config",))?;
        let m: i64 = hnsw_config.call_method1("get", ("M", 16i64))?.extract()?;
    ...
    fn upload_batch(
        ...
        for i in 0..ids.len() {
            ...
            let vec_bytes: Vec<u8> = vec_list.iter().flat_map(|f| f.to_le_bytes()).collect();
    ...
    fn upload_all(
        ...
        let hnsw_config = upload_params_bound
            .getattr("__getitem__")?
            .call1(("hnsw_config",))?;
        let m: i64 = hnsw_config.call_method1("get", ("M", 16i64))?.extract()?;
        let efc: i64 = hnsw_config
    ...
    /// Compound format has vectors.npy for vectors (payloads.jsonl is ignored for VectorSets).
    fn upload_from_compound(
        ...
        let (all_ids, all_vectors) = py
            .allow_threads(|| read_compound_vectors_only(&dir_path_owned, normalize))
            .map_err(|e| {
