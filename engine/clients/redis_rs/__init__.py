from vector_db_benchmark_rs import (
    RustRedisConfigurator as _RustConfigurator,
    RustRedisUploader as _RustUploader,
    RustRedisSearcher as _RustSearcher,
)
...
class RustRedisConfigurator(BaseConfigurator):
    """Rust-backed RediSearch configurator."""
    ...
    def recreate(self, dataset, collection_params):
        self._rust.recreate(dataset, collection_params)

    def configure(self, dataset):
        return self._rust.configure(dataset)
...
class RustRedisUploader(BaseUploader):
    """Rust-backed RediSearch uploader. Delegates upload_batch to Rust."""
...
class RustRedisSearcher(BaseSearcher):
    """Rust-backed RediSearch searcher. search_all runs entirely in Rust."""
